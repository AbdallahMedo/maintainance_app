...
exports.resetPasswordEmail = async (req, res) => {
  try {
      ...
      return res.send(`
        ...
        <head>
          ...
          <style>
            ...
            .card { background: white; padding: 40px; border-radius: 20px; text-align: center; max-width: 500px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); }
      ...
      return res.send(`
        ...
        <head>
          ...
          <style>
            ...
            .icon { font-size: 60px; margin-bottom: 20px; }
            h2 { color: #dc2626; margin-bottom: 15px; }
            p { color: #64748b; line-height: 1.6; margin-bottom: 10px; }
            .error-detail { background: #fef2f2; padding: 15px; border-radius: 8px; margin: 20px 0; color: #991b1b; font-size: 14px; }
            a { display: inline-block; margin-top: 20px; padding: 12px 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; border-radius: 8px; font-weight: 600; }
          </style>
        </head>
        <body>
          <div class="card">
            ...
            <div class="error-detail">Error: ${err.message}</div>
    ...
    // ✅ SUCCESS PAGE — message only, no buttons, no links
    res.send(`
      <!DOCTYPE html>
      <html>
      <head>
...
exports.renderResetPasswordPage = (req, res) => {
  ...
  // ✅ HTML-encode the token to prevent it from leaking visually on the page
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .form-group input { width: 100%; padding: 14px 45px 14px 45px; border-radius: 10px; border: 2px solid #e2e8f0; font-size: 15px; transition: all 0.3s ease; background: #f8fafc; }
  .form-group input:focus { outline: none; border-color: #667eea; background: white; box-shadow: 0 0 0 4px rgba(102, 126, 234, 0.1); }
  .input-icon { position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #94a3b8; pointer-events: none; }
  .toggle-password { position: absolute; right: 15px; top: 50%; transform: translateY(-50%); background: none; border: none; cursor: pointer; color: #94a3b8; padding: 5px; display: flex; align-items: center; justify-content: center; transition: color 0.3s ease; }
  .toggle-password:hover { color: #667eea; }
  .password-strength { margin-top: 10px; font-size: 13px; }
  .strength-bar { height: 4px; background: #e2e8f0; border-radius: 2px; overflow: hidden; margin-bottom: 5px; }
  .strength-bar-fill { height: 100%; width: 0%; transition: all 0.3s ease; border-radius: 2px; }
  .strength-text { color: #64748b; font-size: 12px; }
  .requirements { background: #f8fafc; border-radius: 10px; padding: 15px; margin-top: 20px; font-size: 13px; }
  .requirements p { color: #334155; font-weight: 600; margin-bottom: 10px; }
  .requirements ul { list-style: none; padding: 0; }
  .requirements li { color: #64748b; padding: 5px 0; display: flex; align-items: center; }
  .requirements li::before { content: "•"; color: #94a3b8; margin-right: 8px; font-size: 18px; }
  .requirements li.valid { color: #10b981; }
  .requirements li.valid::before { content: "✓"; color: #10b981; }
  button[type="submit"] { width: 100%; padding: 16px; border: none; border-radius: 10px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.4); margin-top: 25px; }
  ...
  .message { margin-top: 20px; padding: 15px; border-radius: 10px; text-align: center; font-size: 14px; display: none; }
  ...
  @media (max-width: 500px) { .container { padding: 40px 25px; } .header h2 { font-size: 24px; } }
</style>
</head>
<body>
<div class="container">
...
<script>
  ...
  newPasswordInput.addEventListener('input', function () {
    ...
    if (strength <= 2) { strengthBar.style.background = '#ef4444'; strengthText.textContent = 'Weak password'; strengthText.style.color = '#ef4444'; }
    else if (strength <= 4) { strengthBar.style.background = '#f59e0b'; strengthText.textContent = 'Medium password'; strengthText.style.color = '#f59e0b'; }
    else { strengthBar.style.background = '#10b981'; strengthText.textContent = 'Strong password'; strengthText.style.color = '#10b981'; }
  });
  ...
  document.getElementById('resetForm').addEventListener('submit', function (e) {
    ...
    if (newPassword.length < 8) { messageDiv.className = 'message error'; messageDiv.textContent = 'Password must be at least 8 characters long'; return; }
    if (!/[A-Z]/.test(newPassword)) { messageDiv.className = 'message error'; messageDiv.textContent = 'Password must contain at least one uppercase letter'; return; }
    if (!/[a-z]/.test(newPassword)) { messageDiv.className = 'message error'; messageDiv.textContent = 'Password must contain at least one lowercase letter'; return; }
    if (!/[0-9]/.test(newPassword)) { messageDiv.className = 'message error'; messageDiv.textContent = 'Password must contain at least one number'; return; }
    if (newPassword !== confirmPassword) { messageDiv.className = 'message error'; messageDiv.textContent = 'Passwords do not match'; return; }
